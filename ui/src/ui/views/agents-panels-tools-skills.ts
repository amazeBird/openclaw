...
function renderEffectiveToolBadge(tool: {
  ...
  if (tool.source === "plugin") {
    return tool.pluginId
      ? t("agentsPage.tools.effectiveConnectedWith", { id: tool.pluginId })
      : t("agentsPage.tools.effectiveConnected");
  }
  if (tool.source === "channel") {
    return tool.channelId
      ? t("agentsPage.tools.effectiveChannelWith", { id: tool.channelId })
      : t("agentsPage.tools.effectiveChannel");
  }
  return t("agentsPage.tools.effectiveBuiltin");
}
...
export function renderAgentTools(params: {
  ...
  const profileSourceRaw = agentTools.profile
    ? "agent override"
    : globalTools.profile
      ? "global default"
      : "default";
  const profileSource =
    profileSourceRaw === "agent override"
      ? t("agentsPage.tools.profileSourceAgent")
      : profileSourceRaw === "global default"
        ? t("agentsPage.tools.profileSourceGlobal")
    ...
    <section class="card">
      <div class="row" style="justify-content: space-between; flex-wrap: wrap;">
        <div style="min-width: 0;">
          <div class="card-title">${t("agentsPage.tools.accessTitle")}</div>
          <div class="card-sub">
            ${t("agentsPage.tools.accessSub")}
            <span class="mono">${t("agentsPage.tools.enabledFraction", {
        ...
        <div class="row" style="gap: 8px; flex-wrap: wrap;">
          ...
          <button class="btn btn--sm" ?disabled=${params.configLoading} @click=${params.onConfigReload}>
            ${t("agentsPage.tools.reloadConfig")}
          </button>
          <button
          ...
          >
            ${params.configSaving ? t("agentsPage.tools.saving") : t("agentsPage.tools.save")}
          </button>
        </div>
      </div>
      ...
      ${
        hasAgentAllow
          ? html`
              <div class="callout info" style="margin-top: 12px">
                ${t("agentsPage.tools.allowlistInfo")}
              </div>
            `
          : nothing
      }
      ${
        hasGlobalAllow
          ? html`
              <div class="callout info" style="margin-top: 12px">
                ${t("agentsPage.tools.globalAllowInfo")}
              </div>
            `
          : nothing
      }
      ${
        params.toolsCatalogLoading && !params.toolsCatalogResult && !params.toolsCatalogError
          ? html`
              <div class="callout info" style="margin-top: 12px">${t("agentsPage.tools.loadingCatalog")}</div>
            `
          : nothing
      }
      ${
      ...
      <div style="margin-top: 18px;">
        ...
        <div class="card-sub">
          ${t("agentsPage.tools.availableNowSub")}
        ...
        ${
          !params.runtimeSessionMatchesSelectedAgent
            ...
            : params.toolsEffectiveLoading &&
              ...
              ? html`
                  <div class="callout info" style="margin-top: 12px">${t("agentsPage.tools.loadingEffective")}</div>
                `
              : params.toolsEffectiveError
                ? html`
                    <div class="callout info" style="margin-top: 12px">
                      ${t("agentsPage.tools.effectiveError")}
                    </div>
                  `
                : (params.toolsEffectiveResult?.groups?.length ?? 0) === 0
                  ? html`
                      <div class="callout info" style="margin-top: 12px">
                        ${t("agentsPage.tools.noToolsSession")}
                      </div>
                    `
                  : html`
...
export function renderAgentSkills(params: {
  ...
  const filtered = filter
    ...
    : rawSkills;
  const groups = groupSkills(filtered).map((g) => {
    const key = `agentsPage.skillGroups.${g.id}`;
    const loc = t(key);
    return { ...g, label: loc !== key ? loc : g.label };
  });
  ...
  return html`
    <section class="card">
      <div class="row" style="justify-content: space-between; flex-wrap: wrap;">
        <div style="min-width: 0;">
          <div class="card-title">${t("agentsPage.skills.title")}</div>
          <div class="card-sub">
            ${t("agentsPage.skills.sub")}
            ${
              totalCount > 0
        ...
        <div class="row" style="gap: 8px; flex-wrap: wrap;">
          <div class="row" style="gap: 4px; border: 1px solid var(--border); border-radius: var(--radius-md); padding: 2px;">
            ...
            >
              ${t("agentsPage.skills.reset")}
            </button>
          </div>
          <button class="btn btn--sm" ?disabled=${params.configLoading} @click=${params.onConfigReload}>
            ${t("agentsPage.skills.reloadConfig")}
          </button>
          <button class="btn btn--sm" ?disabled=${params.loading} @click=${params.onRefresh}>
            ${params.loading ? t("agentsPage.skills.loading") : t("agentsPage.skills.refresh")}
          </button>
          <button
            class="btn btn--sm primary"
            ?disabled=${params.configSaving || !params.configDirty}
            @click=${params.onConfigSave}
          >
            ${params.configSaving ? t("agentsPage.skills.saving") : t("agentsPage.skills.save")}
          </button>
        </div>
      </div>
      ...
      ${
        !params.configForm
          ? html`
              <div class="callout info" style="margin-top: 12px">
                ${t("agentsPage.skills.loadConfig")}
              </div>
            `
          : nothing
      }
      ${
        usingAllowlist
          ? html`
              <div class="callout info" style="margin-top: 12px">${t("agentsPage.skills.customAllowlist")}</div>
            `
          : html`
              <div class="callout info" style="margin-top: 12px">
      ...
      ${
        !reportReady && !params.loading
          ? html`
              <div class="callout info" style="margin-top: 12px">
                ${t("agentsPage.skills.loadSkillsHint")}
              </div>
            `
          : nothing
      }
      ...
      <div class="filters" style="margin-top: 14px;">
        <label class="field" style="flex: 1;">
          <span>${t("agentsPage.skills.filter")}</span>
          <input
            .value=${params.filter}
        ...
        <div class="muted">${t("agentsPage.skills.shown", { count: String(filtered.length) })}</div>
      </div>

      ${
        filtered.length === 0
          ? html`
              <div class="muted" style="margin-top: 16px">${t("agentsPage.skills.noneFound")}</div>
            `
          : html`
  ...
  return html`
    <div class="list-item agent-skill-row">
      <div class="list-main">
        ...
        ${
          missing.length > 0
            ? html`<div class="muted" style="margin-top: 6px;">${t("agentsPage.skills.missing")} ${missing.join(", ")}</div>`
            : nothing
        }
        ${
          reasons.length > 0
            ? html`<div class="muted" style="margin-top: 6px;">${t("agentsPage.skills.reason")} ${reasons.join(", ")}</div>`
            : nothing
        }
      </div>
      <div class="list-meta">
